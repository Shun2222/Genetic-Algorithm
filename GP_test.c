...
int ObjFunc(int i){
    ...
    int a = (int)(log(SPACEMAX*10)/log(10))+1;
    int w = (int)(pow(10, a));
    return star*w  + sumcoin;//starが多いほど良い　starが同じ枚数ならcoinの枚数が多いほど良い　coinの枚数の最大でも3桁と考えた
}
...
int TournamentSelect(){//nは2^i
    int i, rand, max=0, t=3;//tは一つのトーナメントに参加する遺伝子の数
...
void Generation(int gen, int select_type){
    ...
    if(select_type==0 || select_type==1){
        for(i=0; i<n_gen; i++){
            ...
            //交叉する遺伝子の選択
            if(select_type==0){
                printf("Selected by Roulette\n");
                parent1 = RouletteSelect();
                parent2 = RouletteSelect();
            }else{
                printf("Selected by Tournament\n");
...
void main(int argc, char** argv){
    ...
    printf("[0] RouletteSelect\n[1] TournamentSelect\n[2] ExpectedValueSelect\n[3] RankingSelect (Proportional)\n[4] RankingSelect (Inverse proportional)\n[5] RankingSelect (Quadratic function)\n");
    scanf("%d", &select_type);
    if(select_type<0 || select_type>5) select_type=0;
