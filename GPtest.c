...
int ObjFunc(int i){
    ...
    for(j=0; j<LEN_CHROM; j++){
        ...
        if(sumspace>SPACEMAX) sumspace -= SPACEMAX;
    }
...
int TournamentSelect(){//nは2^i
    int i, rand, max=0, t=10;//tは一つのトーナメントに参加する遺伝子の数
...
int *RankingSelect(){
    int i, j, k=0;
    int rank, rank_max=10;//rank:順位iの遺伝子のindex rank_max:選択される最低順位
    int num, n=(int)(POP_SIZE*GEN_GAP);//num:選択される回数　n:親の数
    if(n%2 != 0) n+=1;//ペアーを考慮
    int selected[n];//selectedが選択されたもの
    double a, b;

    a = -2*(double)n/(double)(rank_max*rank_max); b = 2*(double)n/(double)rank_max;//1~rank_maxの選択回数の合計=親の数になるように調整
    for(i=1; (i<rank_max+1 && k<n); i++){
        rank = Ranking(i);//適応度の順位i番目
        num = (int)(a*(i-1)+b);//選択個数　比例関係順位が高いほど選択個数が多い
        //printf("i:%d rank:%d num:%d k:%d\n", i, rank, num, k);
        //selectにrankをnum個保存
        for(j=0; j<num; j++){
            selected[k] = rank;
            k++;
...
void Generation(int gen, int select_type){
    int parent1, parent2;
    int *parents;
    int child1, child2;
    int n_gen;
    ...
    if(select_type==0 || select_type==1){
        ...
        (select_type==2)? (parents=RankingSelect()) : (parents=ExpectedValueSelect());
        for(i=0; i<n_gen; i++){
        //Statistics(gen);
        //交叉する遺伝子の選択
            parent1 = parents[2*i];
...
void main(int argc, char** argv){
    ...
    printf("[0] RouletteSelect\n[1] TournamentSelect\n[2] RankingSelect\n[3] ExpectedValueSelect\n");
    scanf("%d", &select_type);
    if(select_type!=0 && select_type!=1 && select_type!=2 && select_type!=3) select_type=0;
...
coin:0 star:0 action:10 space:0
coin:10 star:0 action:3 space:10
coin:10 star:0 action:7 space:13
coin:20 star:0 action:6 space:20
coin:10 star:1 action:6 space:26
coin:20 star:1 action:3 space:32
coin:30 star:1 action:10 space:35
coin:30 star:1 action:7 space:3
coin:40 star:1 action:9 space:10
coin:50 star:1 action:7 space:19
